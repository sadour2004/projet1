...
export function OptimizedAdjustmentsHistory() {
  ...
  // Memoize filtered adjustments to prevent unnecessary recalculations
  const filteredAdjustments = useMemo(() => {
    ...
    return filtered
  }, [adjustments, searchQuery, dateFilter])
  ...
  const stats = useMemo(() => {
    ...
    const increases = adjustments.filter((adj) => adj && adj.qty > 0).length
    const decreases = adjustments.filter((adj) => adj && adj.qty < 0).length
    const totalQuantity = adjustments.reduce((sum, adj) => {
      return sum + (adj?.qty || 0)
    }, 0)
  ...
  useEffect(() => {
    const fetchAdjustments = async () => {
      try {
        ...
        params.append('limit', '100') // Increased limit for better history view

        const response = await fetch(`/api/movements?${params.toString()}`)
        
        if (response.ok) {
          const data = await response.json()
          setAdjustments(data.movements || [])
        } else {
          const errorData = await response.json()
          throw new Error(errorData.error || 'Failed to fetch adjustments')
        }
      } catch (err) {
  ...
  const getAdjustmentType = (qty: number) => {
    return qty > 0 ? 'increase' : 'decrease'
  }

  const getAdjustmentBadge = (qty: number) => {
    const type = getAdjustmentType(qty)
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Augmentations</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">Diminutions</p>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className={`text-2xl font-bold ${stats.totalQuantity >= 0 ? 'text-green-600' : 'text-red-600'}`}>
